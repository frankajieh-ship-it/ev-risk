/**
 * POST /api/receipt/upgrade-dev
 *
 * Development-only inline receipt upgrade.
 * Runs the full AI generation synchronously inside a Next.js API route
 * so the verdict works without needing the Netlify background function.
 *
 * Blocked entirely in production (NODE_ENV === "production").
 */
...
export async function POST(req: NextRequest) {
  ...
  const secret = req.headers.get("x-upgrade-secret");
  if (secret !== process.env.UPGRADE_SECRET) {
  ...
  const { receipt_id, receipt_token, input, features, ip_hash, client_ip, t0 } = payload;

  if (!receipt_id || !receipt_token || !input) {
    return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
  }

  // Mark as generating so the poller shows progress
  ...
  try {
    ...
    if (!validation.sanitized && validation.errors.length > 0 && validation.lintErrors.length === 0) {
      ...
      console.log(`[upgrade-dev] Schema fail for ${receipt_id}`);
    ...
    if (finalReceipt.listing_signals && Array.isArray(finalReceipt.listing_signals) && finalReceipt.listing_signals.length > 0) {
      try {
        ...
        console.error("[upgrade-dev] Scoring error:", e);
      }
    }
    ...
    console.log(`[upgrade-dev] ✓ ${receipt_id} upgraded via ${usedProvider} in ${Date.now() - t0}ms, verdict=${aiVerdict}`);
    return NextResponse.json({ ok: true });

  } catch (err) {
    console.error("[upgrade-dev] Fatal error:", err);
    await supabase.from("receipts").update({ generation_status: "failed" }).eq("id", receipt_id);
