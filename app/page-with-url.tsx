...
export default function Home() {
  ...
  const handleExtractListing = async () => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        model: data.make && data.model ? `${data.make} ${data.model}` : prev.model,
        year: data.year || prev.year,
        trim: data.trim || prev.trim,
        currentMileage: data.mileage || prev.currentMileage,
      }));
                ...
                <div className="ml-3 flex-1">
                  ...
                  {extractionWarnings.length > 0 && (
                    <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-xs font-semibold text-yellow-900 mb-1">Heads up:</p>
