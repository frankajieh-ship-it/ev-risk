...
export default function WorkspaceLayout({
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-1)] flex">
      {/* Sidebar */}
      <aside className="w-60 bg-white border-r border-[var(--border-subtle)] flex flex-col shrink-0 hidden md:flex">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-[0.8125rem] font-medium transition-colors ${
                  isActive
                    ? "bg-[var(--surface-2)] text-gray-900"
                    : "text-gray-500 hover:bg-[var(--surface-1)] hover:text-gray-900"
                }`}
              >
                <item.icon className={`w-4 h-4 ${isActive ? "text-gray-700" : "text-gray-400"}`} />
                {item.label}
              </Link>
            );
          })}
      ...
      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-[var(--border-subtle)] px-4 py-3 flex items-center gap-3">
