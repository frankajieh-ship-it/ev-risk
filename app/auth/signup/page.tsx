...
import { ArrowLeft, Mail, Lock, User, Loader2, Check, Eye, EyeOff } from "lucide-react";
...
function SignupForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // If Supabase email confirmation is disabled, user is auto-signed in
    // and useAuth will pick up the session → redirect via useEffect above.
    // If confirmation is enabled, show the confirmation state.
    setDone(true);
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSignup} className="space-y-3">
        ...
        <button
          type="submit"
          disabled={loading || googleLoading || !email || !password || !displayName}
