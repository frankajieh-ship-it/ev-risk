...
import { TrendingDown, ExternalLink, CircleDollarSign, Info, Calculator, ShieldCheck, AlertTriangle, XCircle } from "lucide-react";
...
export default function AuctionBidGuidanceCard({ result, askingPrice, currentBid, vin }: AuctionBidGuidanceCardProps) {
  ...
  return (
    <div className={`rounded-2xl border p-5 space-y-4 ${gradeBg}`}>
      ...
      {/* Repair Risk */}
      <div className={`rounded-xl border p-3 flex items-center justify-between ${repairRiskBg}`}>
        <div className="flex items-center gap-2">
          <RepairRiskIcon className={`w-4 h-4 ${repairRiskText}`} />
          <div>
            <p className="text-xs font-semibold text-gray-600">Repair Risk</p>
            <p className="text-xs text-gray-500">{score}/100 structural score</p>
          </div>
        </div>
        <p className={`text-xl font-bold ${repairRiskText}`}>{repairRiskLabel}</p>
      </div>
